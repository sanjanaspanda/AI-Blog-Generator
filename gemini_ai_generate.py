...
def generate_blog_post(topic):
    ...
    return json.loads(rf"{text}", strict=False)
...
def generate_ideas_for_blog(categories, existing_topics):
    ...
    for category in categories:
        PROMPT_IDEAS_V1 = f"""
        Generate 1 unique and highly engaging blog post ideas related to {category["name"]}
        ...
        - Each idea should have a unique title and content  
        - Idea will be used as a topic or heading so keep it as short and catchy as possible
        ...
        print(text)
        ideas.append(json.loads(text, strict=False))
    return ideas
